...
func (w *StateWriterV3) UpdateAccountData(address common.Address, original, account *accounts.Account) error {
	...
	if original.Incarnation > account.Incarnation {
		//del, before create: to clanup code/storage
		pc, err := w.rs.domains.LatestCode(address[:])
		if err != nil {
			return err
		}
		if len(pc) > 0 {
			if err := w.rs.domains.DomainDel(kv.CodeDomain, address[:], nil, pc); err != nil {
				return err
			}
		}
		if err := w.rs.domains.DomainDelPrefix(kv.StorageDomain, address[:]); err != nil {
...
func (r *StateReaderV3) ReadAccountStorage(address common.Address, incarnation uint64, key *common.Hash) ([]byte, error) {
	...
	if r.trace {
		if enc == nil {
			fmt.Printf("ReadStorage [%x] [%x] => [empty], txNum: %d\n", address, key.Bytes(), r.txNum)
		} else {
			fmt.Printf("ReadStorage [%x] [%x] => [%x], txNum: %d\n", address, key.Bytes(), enc, r.txNum)
		}
	}
	return enc, nil
}
